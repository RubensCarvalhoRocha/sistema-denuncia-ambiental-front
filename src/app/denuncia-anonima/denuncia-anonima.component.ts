...
@Component({
  ...
  styleUrls: ['./denuncia-anonima.component.scss']
})
export class DenunciaAnonimaComponent implements OnInit {
    ...
    sendForm(): void {
        ...
        if (this.denunciaForm.valid) {
          ...
          this.denunciaAnonimaService.postDenuncia(this.denunciaForm)
            .pipe(
              // Trata possíveis erros na chamada HTTP
              catchError(error => {
                console.error('Error sending the form:', error);
                // Pode mostrar uma mensagem de erro para o usuário se desejar
                // this.mostrarToast('Erro ao enviar a denúncia. Tente novamente mais tarde.', 'Erro', 'error');
                return [];
              })
            )
            .subscribe(response => {
              // Exibe uma mensagem de sucesso (ou de erro) para o usuário
            //   this.mostrarToast(response.message, response.success ? 'Sucesso' : 'Erro', response.success ? 'success' : 'error');
        ...
        //   this.mostrarToast('Preencha todos os campos obrigatórios.', 'Atenção');
        }
      }
